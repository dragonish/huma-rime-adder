...
- "导入词库文件"按钮：用于批量从词库文件(每行一个词条)里导入词条并自动进行编码。
- "校验三简词"按钮：用于校验是否存在与常用单字重复编码的三简词或者多个相同编码的三简词，此外允许继续执行自动处理三简词操作。
- "校验拆分滤镜"按钮：用于校验拆分滤镜是否完整收录码表中的单字。
- "整理拼音码表"按钮：用于移除拼音码表中重复的编码词条，并重写拼音码表文件。
- "整理拼音滤镜"按钮：用于根据码表中的词组重新生成对应拼音提示，并覆盖及重写拼音词组滤镜配置文件。
- "强制退出"按钮：用于强制退出当前程序，不进行任何写入与保存文件等操作。
